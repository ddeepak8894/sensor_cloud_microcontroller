...
void loop() {
  ...
  const unsigned long interval = 200; // 2 seconds interval
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (fb) {
      sendImageOverWebSocket(fb->buf, fb->len); // Send captured image over WebSocket
...
void sendImageOverWebSocket(uint8_t* imgData, size_t imgSize) {
  if (websocketclient.available()) {
    ...
    String encodedImage = base64::encode(imgData, imgSize);

    String eventName = "messageSendToUser";
    String serializedJson = "{\"message\": \"" + encodedImage + "\"}";

    String messageToSend = "42[\"" + String(eventName) + "\", " + serializedJson + "]";

    websocketclient.send(messageToSend); // Sending the constructed message
